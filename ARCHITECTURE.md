...
| Path | Purpose |
|---|---|
| `src/core/` | Engine core (`Core`, `EventBus`, `WorkerBridge`) |
| `src/plugins/` | Built-in `EnginePlugin` implementations |
| `src/workers/` | Standalone Web Worker scripts (no Pixi.js / DOM deps) |
...
| `PathfindingManager.ts` | `pathfinding` | A* pathfinding with weighted terrain, LRU cache, dynamic obstacles (tag-filtered), `fallbackToNearest`, `smoothPath`, and optional Worker offloading via `pathfinding/find:async` |
...
| File | Description |
|---|---|
| `Core.ts` | Engine host — wires EventBus, plugin lifecycle |
| `EventBus.ts` | Event bus — sync/async emit, namespace scoping |
| `WorkerBridge.ts` | Generic main↔Worker bridge; see [§16 WorkerBridge](#16-workerbridge) |

Worker scripts in `src/workers/`:

| File | Description |
|---|---|
| `pathfinding.worker.ts` | Standalone A* worker used by `PathfindingManager` when `workerUrl` is provided; no Pixi.js / DOM dependencies |
...
- Worker scripts in `src/workers/` must not import from `src/plugins/` or `src/rendering/`.
...
physics/tilemap:set    ──► _buildGrid()              rebuild full cost grid
                           _syncGridToWorker()        transfer Float32Array to Worker (if configured)
tilemap/set-tile       ──► _updateCell(row,col)       O(1) single-cell update
                           Worker 'tile:update'       forward new cost to Worker
pathfinding/find       ──► _find(params)              synchronous A* (main thread, always available)
pathfinding/find:async ──► WorkerBridge.run('find')   async A* in Worker (falls back to sync if not ready)

MainThread                          Worker Thread
  │                                        │
  │ postMessage({id:'init', grid})  ──────►│  rebuild internal grid
  │ ◄─────────────────────────────────────│  reply {id, result:{ok:true}}
  │                                        │
  │ postMessage({id, type:'find'})  ──────►│  A* computation
  │ ◄─── postMessage({id, result})  ───────│
  │ resolve Promise / write output         │
```
...
- **Dynamic obstacles** — when `includeDynamicObstacles: true` is set, entity positions are
  queried via `entity/query` on the **main thread** (EventBus access is required).  When
  Worker mode is active, the collected `[row, col][]` array is packed into the `find` payload
  and forwarded to the Worker.  Use `tagFilter` to restrict which entities count as obstacles.
  Dynamic-obstacle results are never cached.
- **Path cache** — static A* results are stored in a 512-entry **LRU** `Map` keyed by
  ...
  `tilemap/set-tile` (which also updates the affected grid cell in O(1)).  In Worker mode the
  cache lives inside the Worker; the main thread sends `{type:'cache:clear'}` messages on
  all standard invalidation events.
...
| `pathfinding/find:async`   | ✓ async | Same params as `pathfinding/find`; runs in Worker when `workerUrl` is set, otherwise falls back to synchronous A* |
...
### 15.4 `pathfinding/find` / `pathfinding/find:async` Parameters
...
new PathfindingManager({
  directions: 4 | 8,     // 4: cardinal only; 8: cardinal + diagonal (default)
  workerUrl: string | URL // optional — enables Worker offloading for find:async
})
```

| Option       | Type             | Default | Description |
|--------------|------------------|---------|-------------|
| `directions` | `4 \| 8`         | `8`     | Movement directions — cardinal only or cardinal + diagonal |
| `workerUrl`  | `string \| URL`  | —       | URL of the compiled `pathfinding.worker` script.  When set, `init()` creates a `WorkerBridge` and transfers the cost grid as a `Float32Array` on every `physics/tilemap:set`.  `destroy()` calls `bridge.terminate()`.  Omit entirely for main-thread-only mode. |
...
// ── Worker mode (Vite / bundler project) ─────────────────────────────
const { core: wCore } = await createEngine({
  plugins: [
    new EntityManager(),
    new KinematicPhysicsAdapter(),
    new TilemapManager(),
...
// Sync API unchanged — still runs on the main thread:
const { output: syncOut } = wCore.events.emitSync('pathfinding/find', { from, to });

// Async API — offloaded to Worker; falls back to sync if Worker not yet ready:
const { output: asyncOut } = await wCore.events.emit<PathfindingFindParams, PathfindingFindOutput>(
...
The grid is kept in sync with two event sources (both main-thread and Worker):

| Event                   | Main-thread handler | Worker message sent |
|-------------------------|---------------------|---------------------|
| `physics/tilemap:set`   | Full grid rebuild (`O(rows × cols)`) + cache clear | `init` — transfers `Float32Array` grid (zero-copy) |
| `tilemap/set-tile`      | Single-cell update (`O(1)`) + cache clear | `tile:update {row, col, cost}` (only after Worker ack'd `init`) |
| `pathfinding/cache:clear` | Main-thread cache cleared | `cache:clear` |
| `pathfinding/weight:set` | Grid rebuild + cache clear | `init` — re-transfers updated grid |
...
### 15.8 Worker Offloading (`pathfinding/find:async`)

When `workerUrl` is provided the manager creates a `WorkerBridge` at `init()` time.

**Initialisation flow:**

1. `physics/tilemap:set` fires → main thread rebuilds local cost grid → serialises to
   `Float32Array` (flat row-major) → transfers to Worker via `postMessage` with the
   `Transferable` list (zero-copy).
2. Worker reconstructs its internal grid and replies `{id, result:{ok:true}}`.
3. Bridge resolves; `_workerReady` is set to `true`.
4. Subsequent `pathfinding/find:async` calls are routed to the Worker.

**Fallback behaviour:** while `_workerReady` is `false` (Worker not yet ack'd, or no
`workerUrl` provided), `pathfinding/find:async` runs the synchronous A* on the main thread
transparently — callers never need to handle the unready case explicitly.

**Dynamic obstacles in Worker mode:** `entity/query` can only run on the main thread.  The
handler collects the blocked `[row, col]` pairs before sending the `find` task, then packs
them into the payload for the Worker.

**Graceful degradation:** if the Worker emits an `onerror` event or replies with `{error:…}`,
the bridge rejects the corresponding promise.  The error is logged via `console.warn`; the
synchronous `pathfinding/find` API is always available as a fallback.

---

## 16. WorkerBridge

`WorkerBridge<I, O>` is a **generic, reusable** main↔Worker bridge in `src/core/WorkerBridge.ts`.
It is used internally by `PathfindingManager` but can be imported and used by any plugin that
needs to offload heavy computation off the main thread.

### 16.1 Protocol

Every message pair uses the `WorkerTask` / `WorkerResult` types from `src/types/worker.ts`:

```ts
// Sent main → Worker
interface WorkerTask<P = unknown> {
  readonly id: string;    // unique correlation ID
  readonly type: string;  // command discriminator
  readonly payload: P;
}

// Sent Worker → main
interface WorkerResult<R = unknown> {
  readonly id: string;     // mirrors the originating task id
  readonly result?: R;     // present on success
  readonly error?: string; // present on failure
}
```

The Worker script must echo back the same `id` it received so that the bridge can resolve
the correct `Promise`.

### 16.2 API

```ts
new WorkerBridge<I, O>(workerUrl: string | URL, options?: WorkerBridgeOptions)
```

| Member | Type | Description |
|--------|------|-------------|
| `run(type, payload, transfer?)` | `Promise<O>` | Send a task and await the Worker reply.  Pass `transfer` for zero-copy `Transferable` objects (e.g. `ArrayBuffer`). |
| `terminate()` | `void` | Terminate the Worker immediately.  All in-flight and queued promises are rejected.  Subsequent `run()` calls reject immediately.  Idempotent. |
| `isTerminated` | `boolean` | `true` after `terminate()` is called |
| `pendingCount` | `number` | Number of tasks currently awaiting a reply |

`WorkerBridgeOptions`:

| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `maxConcurrent` | `number` | `0` | Maximum in-flight tasks at once.  `0` = unlimited.  When the limit is reached, new `run()` calls are queued and dispatched as earlier tasks complete. |

### 16.3 Error Handling

| Scenario | Behaviour |
|----------|-----------|
| Worker replies `{ id, error: "…" }` | Corresponding `run()` promise is rejected with that message |
| Worker fires `onerror` (uncaught exception) | All in-flight and queued promises are rejected; Worker is **not** automatically restarted |
| `terminate()` called with pending tasks | All pending and queued promises are rejected with `"Worker terminated."` |

### 16.4 Usage

```ts
import { WorkerBridge } from 'inkshot-engine';
import type { WorkerTask, WorkerResult } from 'inkshot-engine';

// ── Create a bridge (Vite / bundler) ────────────────────────────────
const bridge = new WorkerBridge<{ input: number[] }, { sum: number }>(
  new URL('./my-heavy.worker.js', import.meta.url),
  { maxConcurrent: 2 },  // at most 2 tasks in-flight simultaneously
);

// ── Send a task ──────────────────────────────────────────────────────
const result = await bridge.run('sum', { input: [1, 2, 3, 4, 5] });
console.log(result.sum); // 15

// ── Zero-copy transfer of a large buffer ────────────────────────────
const buf = new Float32Array(1_000_000);
// buf.buffer is transferred to the Worker (zero-copy); do not use buf after this.
await bridge.run('process', { data: buf.buffer } as unknown as Record<string, unknown>, [buf.buffer]);

// ── Teardown ─────────────────────────────────────────────────────────
bridge.terminate();
```

Worker side (`my-heavy.worker.ts`):

```ts
import type { WorkerTask, WorkerResult } from 'inkshot-engine';

self.onmessage = (evt: MessageEvent<WorkerTask>) => {
  const { id, type, payload } = evt.data;
  try {
    if (type === 'sum') {
      const { input } = payload as { input: number[] };
      const sum = input.reduce((a, b) => a + b, 0);
      const reply: WorkerResult<{ sum: number }> = { id, result: { sum } };
...
`WorkerBridge`, `WorkerBridgeOptions`, `WorkerTask`, and `WorkerResult` are all exported from
the package root:

```ts
import { WorkerBridge } from 'inkshot-engine';
import type { WorkerBridgeOptions, WorkerTask, WorkerResult } from 'inkshot-engine';
```

---

## 17. UI Widget System (UIManager)
...
| `dialoguebox` | Dialogue rendering widget (see §18) |
...
### 17.4 Custom Widget Registration
...
## 18. Dialogue System (DialogueManager)
...
### 18.6 `parseDialogueMarkup` — Public API
...
## 19. Script System (ScriptManager)
...
### 20.3 `actor/spawned` Trigger Behaviour
