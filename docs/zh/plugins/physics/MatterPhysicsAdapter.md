...
const { core } = await createEngine({
  plugins: [/* MatterPhysicsAdapter（`physics`） */],
});
```
...
| 事件 | 說明 | 方向 |
|------|------|------|
| `physics/body-add` | physics/body-add 操作 | 監聽 / 發出 |
| `physics/body-remove` | physics/body-remove 操作 | 監聽 / 發出 |
| `physics/collide` | physics/collide 操作 | 監聽 / 發出 |
| `physics/update` | physics/update 操作 | 監聽 / 發出 |
...
```ts
// 使用 MatterPhysicsAdapter（`physics`）
